...
static int
traverse_good_cb(BSTreeNode *node)
{
...
static int
traverse_fail_cb(BSTreeNode *node)
{
	debug("KEY: %s", bdata((bstring)node->key));
	traverse_called++;
...
test_traverse()
{
	...
	mu_assert(traverse_called == 2, "Wrong count traveres for fail.");
...
test_delete()
{
	...
	bstring result = BSTree_get(map, &test1);
	mu_assert(result == NULL, "Should delete &test1");

	deleted = (bstring)BSTree_delete(map, &test1);
	mu_assert(deleted == NULL, "Slould get NULL on delete &test1");
	...
	deleted = (bstring)BSTree_delete(map, &test3);
	mu_assert(deleted == NULL, "Should get NULL on delete &test3");
