...
struct tagbstring IN_STR =
    bsStatic("I have ALPHA beta ALPHA and oranges ALPHA");
...
char *test_find_and_scan() {
  ...
  mu_assert(StringScanner_scan(scan, &ALPHA) == -1, "shoudn't find it.");
...
char *test_binstr_performance() {
  ...
  debug("BINSTR COUNT: %lu, END TIME: %d, OPS: %f", find_count, (int)elapsed,
...
char *test_find_performance() {
  ...
  debug("FIND COUNT: %lu, END TIME: %d, OPS: %f", find_count, (int)elapsed,
        (double)find_count / elapsed);
...
char *test_scan_performance() {
  ...
  debug("SCAN COUNT: %lu, END TIME: %d, OPS: %f", find_count, (int)elapsed,
        (double)find_count / elapsed);
